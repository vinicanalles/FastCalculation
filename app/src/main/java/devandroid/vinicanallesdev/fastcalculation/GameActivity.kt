...
import devandroid.vinicanallesdev.fastcalculation.databinding.ActivityGameBinding
...
class GameActivity : AppCompatActivity(), OnPlayGame {
    ...
    override fun onResultGame(result: Result) {
        supportFragmentManager.beginTransaction().replace(R.id.gameFl, ResultFragment.newInstance(result)).commit()
    }
}